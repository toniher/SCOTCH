...
def parse_info(x: str, sep=";", inner_sep=" ") -> dict:
    ...
    for i in x:
        ...
        if len(pair) == 2 and (
            pair[0] in ["gene_name", "gene_id", "transcript_id", "gene_biotype"]
        ):
...
def generate_reference_df(gtf_path):
    ...
    df_gene = (
        df[df.iloc[:, 2] == "gene"]
        .loc[
            :, ["CHR", "START", "END", "gene_id", "gene_name", "gene_biotype", "STRAND"]
        ]
        .reset_index(drop=True)
    )
    df_exon = (
        df[df.iloc[:, 2] == "exon"]
        .loc[:, ["CHR", "START", "END", "gene_id", "gene_name", "transcript_id"]]
        .reset_index(drop=True)
    )
    ...
    df_gene.to_csv(
        outfile_gene, sep="\t", header=False, index=False, quoting=csv.QUOTE_NONE
    )
    df_exon.to_csv(
        outfile_exon, sep="\t", header=False, index=False, quoting=csv.QUOTE_NONE
    )
    # sort
