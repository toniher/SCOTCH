...
def convert_to_gtf(
    ...
    def update_annotation_gene(geneID, gtf_df, geneStructureInformationwNovel):
        gtf_df_sub = gtf_df[
            gtf_df["attribute"].str.contains(f'gene_id "{geneID}"', regex=False)
        ...
        if gtf_df_sub.shape[0] > 0:
            ...
            new_row_gene = [
                [
                    ...
                    "SCOTCH",
                    "gene",
                    geneInfo["geneStart"] + 1,
                    geneInfo["geneEnd"],
                    ".",
                    geneInfo["geneStrand"],
                    ".",
                    f'gene_id "{geneID}"; gene_name "{geneInfo["geneName"]}"',
                ]
            ]
        new_rows_isoforms = []
        for isoform_name, exon_indices in isoformInfo.items():
            ...
            if (
                gtf_df_sub[
                    gtf_df_sub["attribute"].str.contains(
                        f'transcript_id "{isoform_name}"', regex=False
                    )
                ].shape[0]
                == 0
            ):
                ...
                for exon_num, (exon_start, exon_end) in enumerate(
                    merged_exons_isoform, start=1
                ):
                    new_rows_exon = [
                        geneInfo["geneChr"],
                        "SCOTCH",
    ...
    geneStructureInformationwNovel = partition_metagene(
        metageneStructureInformationNovel
    )
...
def summarise_annotation(target, logger):
    ...
    for reference_folder in reference_folders:
        output_pkl = os.path.join(
            reference_folder, "metageneStructureInformationwNovel.pkl"
        )
        ...
        if len(file_names_pkl) > 0 and len(file_names_gtf) > 0:
            ...
            metageneStructureInformationwNovel = dict(
                sorted(
                    metageneStructureInformationwNovel.items(),
                    key=lambda item: get_numeric_key(item[0]),
                )
            )
...
def summarise_auxillary(target, logger):
    ...
    for auxillary_folder in auxillary_folders:
        ...
        df_list = Parallel(n_jobs=-1)(
            delayed(read_file)(file_path) for file_path in file_paths
        )
        ...
        DF = DF.sort_values(
            by=["geneChr", "Read", "priority"], ascending=[True, True, False]
        )
        # Split DF into DF_unique and DF_multiple
        unique_mask = ~DF["Read"].duplicated(keep=False)  # Reads that appear only once
        multiple_mask = DF["Read"].duplicated(
            keep=False
        )  # Reads that appear more than once
...
class ReadMapper:
    def __init__(
        ...
        self.annotation_folder_path_list = [
            os.path.join(target_, "reference") for target_ in target
        ]
        self.annotation_path_single_gene_list = [
            os.path.join(target_, "reference/geneStructureInformation.pkl")
            for target_ in target
        ]
        self.annotation_path_meta_gene_list = [
            os.path.join(target_, "reference/metageneStructureInformation.pkl")
            for target_ in target
        ]
        self.annotation_path_meta_gene_novel_list = [
            os.path.join(target_, "reference/metageneStructureInformationwNovel.pkl")
            for target_ in target
        ]
        self.annotation_path_gtf_novel_list = [
            os.path.join(target_, "reference/gene_annotations_scotch.gtf")
            for target_ in target
        ]
        # bam information path
        self.bamInfo_folder_path_list = [
            os.path.join(target_, "bam") for target_ in target
        ]
        self.bamInfo_pkl_path_list = [
            os.path.join(target_, "bam/bam.Info.pkl") for target_ in target
        ]  # bamInfo_pkl_file
        self.bamInfo2_pkl_path_list = [
            os.path.join(target_, "bam/bam.Info2.pkl") for target_ in target
        ]  # bamInfo2_pkl_file
        self.bamInfo3_pkl_path_list = [
            os.path.join(target_, "bam/bam.Info3.pkl") for target_ in target
        ]  # bamInfo2_pkl_file
        self.bamInfo_csv_path_list = [
            os.path.join(target_, "bam/bam.Info.csv") for target_ in target
        ]
        # parameters
        ...
        if platform != "parse-ont":
            ...
            self.compatible_matrix_folder_path_list = [
                os.path.join(target_, "compatible_matrix") for target_ in target
            ]  # not for parse
            self.read_mapping_path_list = [
                os.path.join(target_, "auxillary") for target_ in target
            ]  # not for parse
            self.qname_dict_list = [
                load_pickle(bamInfo_pkl_path)
                for bamInfo_pkl_path in self.bamInfo_pkl_path_list
            ]
            self.qname_cbumi_dict_list = [
                load_pickle(bamInfo2_pkl_path)
                for bamInfo2_pkl_path in self.bamInfo2_pkl_path_list
            ]
            self.sorted_bam_path_list = None
            self.qname_sample_dict_list = [
                load_pickle(bamInfo3_pkl_path)
                for bamInfo3_pkl_path in self.bamInfo3_pkl_path_list
            ]
        else:
        ...
        self.metageneStructureInformation = load_pickle(
            self.annotation_path_meta_gene_list[0]
        )
        self.metageneStructureInformationwNovel = (
            self.metageneStructureInformation.copy()
        )
    ...
    def read_bam(self, chrom=None):
        ...
        for bam_path in self.bam_path:
            if (
                os.path.isfile(bam_path) == False
            ):  # If it's a folder, find the BAM file based on chrom
                if chrom is not None:
                    bamFile_name = [
                        f
                        for f in os.listdir(bam_path)
                        if f.endswith(".bam") and "." + chrom + "." in f
                    ]
    ...
    def map_reads(self, meta_gene, save=True):
        ...
        if len(Info_multigenes) == 1:
            ...
            if len(Read_novelIsoform) > 0:
                ...
                (
                    Read_novelIsoform_polished,
                    novel_isoformInfo_polished,
                    Read_knownIsoform_polished,
                ) = polish_compatible_vectors(
                    Read_novelIsoform,
                    Read_knownIsoform,
                    n_isoforms,
                    exonInfo,
            ...
            self.metageneStructureInformationwNovel[meta_gene][0][0]["isoformNames"] = (
                self.metageneStructureInformationwNovel[meta_gene][0][0]["isoformNames"]
                + list(novel_isoformInfo_polished.keys())
            )
            self.metageneStructureInformationwNovel[meta_gene][0][0][
                "numofIsoforms"
            ] = self.metageneStructureInformationwNovel[meta_gene][0][0][
                "numofIsoforms"
            ] + len(list(novel_isoformInfo_polished.keys()))
            self.metageneStructureInformationwNovel[meta_gene][0][2].update(
                novel_isoformInfo_polished
            )
            for key in Read_knownIsoform_scores:
                Read_knownIsoform_scores[key].extend(
                    [-1] * len(list(novel_isoformInfo_polished.keys()))
                )
            sample_list = []
            for sample_ind in range(self.nsamples):
                sample_target = self.target[sample_ind]
                (
                    Read_Isoform_compatibleVector_sample,
                    Read_knownIsoform_scores_sample,
                ) = {}, {}
                for readname in list(Read_Isoform_compatibleVector.keys()):
                    if qname_sample_dict[readname] == "sample" + str(sample_ind):
                        Read_Isoform_compatibleVector_sample[readname] = (
                            Read_Isoform_compatibleVector[readname]
                        )
                        if readname in Read_knownIsoform_scores.keys():
                            Read_knownIsoform_scores_sample[readname] = (
                ...
                if save:
                    ...
                    save_compatibleVector_by_gene(
                        ...
                        Read_Isoform_compatibleVector_sample,
                        Read_knownIsoform_scores_sample,
                        self.qname_cbumi_dict_list[sample_ind],
                    ...
                    sample_list.append(
                        {
                            ...
                            "isoforms": colNames,
                            "exonInfo": self.metageneStructureInformationwNovel[
                                meta_gene
                            ][0][1],
                            "isoformInfo": self.metageneStructureInformationwNovel[
                                meta_gene
                            ][0][2],
                        }
                    )
            ...
            geneChr, start, end = summarise_metagene(
                Info_multigenes
            )  # geneChr, start, end
            qname_sample_dict = {}
            results = []
            # self.logger.info(self.qname_dict_list)
            for i, bamFilePysam in enumerate(bamFilePysams):
                reads = bamFilePysam.fetch(
                    geneChr, start, end
                )  # fetch reads within meta gene region
                # process reads metagene
                self.logger.info(i)
                if i not in self.qname_dict_list:
                    # Toniher: Skip if not key
                    self.logger.info("MISSING KEY " + str(i))
                    continue
                for read in reads:
                    readName, readStart, readEnd = (
                        read.qname,
            ...
            (
                Ind,
                Read_novelIsoform_metagene,
                Read_knownIsoform_metagene,
                Read_knownIsoform_metagene_scores,
            ) = [], [], [], []
            ...
            log_ind = [
                ind for ind in range(len(Info_multigenes)) if ind not in unique_ind
            ]
            for index in log_ind:
                for sample_ind in range(self.nsamples):
                    sample_target = self.target[sample_ind]
                    save_compatibleVector_by_gene(
                        geneName=Info_multigenes[index][0]["geneName"],
                        geneID=Info_multigenes[index][0]["geneID"],
                        geneChr=Info_multigenes[index][0]["geneChr"],
                        colNames=None,
                        Read_Isoform_compatibleVector=None,  # set this to None for log
                        Read_knownIsoform_scores=None,
                        qname_cbumi_dict=None,
                        exonInfo=None,
                        isoformInfo=None,
            ...
            for index in unique_ind:
                ...
                for j, i in enumerate(
                    Ind
                ):  # i: gene index; j: index of index---# loop for reads
                    ...
                    if i == index and Read_knownIsoform_metagene[j] is not None:
                        ...
                        Read_knownIsoform_scores[Read_knownIsoform_metagene[j][0]] = (
                            Read_knownIsoform_metagene_scores[j]
                        )
                if len(Read_novelIsoform) > 0:
                    (
                        Read_novelIsoform_polished,
                        ...
                        Read_novelIsoform,
                        Read_knownIsoform,
                        len(Info_multigenes[index][2]),
                        Info_multigenes[index][1],
                        self.small_exon_threshold,
                        self.small_exon_threshold1,
                    )
                else:
                ...
                # update annotation information in self
                self.metageneStructureInformationwNovel[meta_gene][index][0][
                    "isoformNames"
                ] = self.metageneStructureInformationwNovel[meta_gene][index][0][
                    "isoformNames"
                ] + list(novel_isoformInfo_polished.keys())
                ...
                self.metageneStructureInformationwNovel[meta_gene][index][2].update(
                    novel_isoformInfo_polished
                )
                for sample_ind in range(self.nsamples):
                    sample_target = self.target[sample_ind]
                    ...
                    if save:
                        save_compatibleVector_by_gene(
                            ...
                            Read_knownIsoform_scores_sample,
                            self.qname_cbumi_dict_list[sample_ind],
                            self.metageneStructureInformationwNovel[meta_gene][index][
                                1
                            ],
                            self.metageneStructureInformationwNovel[meta_gene][index][
                                2
    ...
    def map_reads_parse(self, meta_gene, save=True):
        ...
        if len(Info_multigenes) == 1:
            ...
            for sample in unique_samples:
                ...
                if save:
                    ...
                    return_sample = {
                        "Read_Isoform_compatibleVector": Read_Isoform_compatibleVector_sample,
                        "isoforms": colNames,
                        "exonInfo": self.metageneStructureInformationwNovel[meta_gene][
                            0
                        ][1],
                        "isoformInfo": self.metageneStructureInformationwNovel[
                            meta_gene
            ...
            for index in log_ind:
                for sample in unique_samples:
                    ...
                    save_compatibleVector_by_gene(
                        ...
                        geneChr=Info_multigenes[index][0]["geneChr"],
                        colNames=None,
                        Read_Isoform_compatibleVector=None,
            ...
            for index in unique_ind:  # loop gene
                ...
                if save:
                    for sample in unique_samples:
                        sample_target = os.path.join(
                            self.target[0], "samples/" + sample
                        )
                        if not os.path.exists(sample_target):
                            os.makedirs(sample_target)
                        (
    ...
    def map_reads_allgenes(
        ...
        if self.parse == False:
            for (
                compatible_matrix_folder_path
            ) in self.compatible_matrix_folder_path_list:
        ...
        if cover_existing:
            ...
            if self.parse:
                self.compatible_matrix_folder_paths = find_subfolder(
                    self.target[0], subfolder="compatible_matrix"
                )
                self.read_mapping_paths = find_subfolder(
                    self.target[0], subfolder="auxillary"
                )
                genes_existing = [
                    file[:-4]
        ...
        for metagene_name, genes_info in self.metageneStructureInformation.items():
            if metagene_name in MetaGenes_job:
                ...
                for gene_info in genes_info:
                    gene = (
                        str(gene_info[0]["geneName"])
                        + "_"
                        + str(gene_info[0]["geneID"])
                    )
        ...
        gene_ids = [
            g_name_id.split("_")[1]
            for g_name_ids in list(MetaGene_Gene_dict.values())
            for g_name_id in g_name_ids
        ]
        gene_ids_pattern = "|".join([f'gene_id "{gene_id}"' for gene_id in gene_ids])
        self.gtf_df_job = self.gtf_df[
            self.gtf_df["attribute"].str.contains(gene_ids_pattern, regex=True)
        ].reset_index(drop=True)
    ...
    def save_annotation_w_novel_isoform(self, total_jobs=1, current_job_index=0):
        ...
        for i in range(len(self.annotation_path_meta_gene_list)):
            if total_jobs > 1:
                file_name_pkl = (
                    self.annotation_path_meta_gene_novel_list[i][:-4]
                    + "_"
                    + str(current_job_index)
                    + ".pkl"
                )
                file_name_gtf = (
                    self.annotation_path_gtf_novel_list[i][:-4]
                    + "_"
                    + str(current_job_index)
                    + ".gtf"
                )
