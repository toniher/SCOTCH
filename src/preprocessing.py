...
def sort_multigeneInfo(Info_multigenes):
    ...
    for info_singlegene in Info_multigenes:
        ...
        if len(isoformInfo) > 0:
            isoformInfo = dict(
                sorted(isoformInfo.items(), key=lambda item: len(item[1]), reverse=True)
            )
...
def read_similarity(df_assign):
    ...
    consistent_matrix = np.tensordot(ones, ones, axes=([1], [1])) + np.tensordot(
        minus_ones, minus_ones, axes=([1], [1])
    )
...
def find_novel(df_assign):
    ...
    for k in range(len(read_groups)):
        if len(read_groups[k]) > 1:
            ...
            count_neg1 = (df == -1).sum()
            assign = [
                (1 if count_1[col] > count_neg1[col] else -1) for col in df.columns
            ]
            ...
            if (
                len(assign) > -sum(assign)
                and isoform_id not in novelisoform_dict.keys()
            ):
...
def polish_compatible_vectors(
    Read_novelIsoform,
    Read_Isoform_compatibleVector,
    n_isoforms,
    exonInfo,
    small_exon_threshold,
    small_exon_threshold1,
):
    ####generate novel isoform annotation
    if len(Read_novelIsoform) == 1:
        Read_Isoform_compatibleVector.append(
            (Read_novelIsoform[0][0], [0] * n_isoforms)
        )
        ...
        novel_dict_assign = {
            readname: readassignment
            for readname, readpct, readassignment in Read_novelIsoform
        }
        ...
        novelisoform_dict, novel_df, novel_df_empty = find_novel_by_chunk(
            df_assign,
            exonInfo,
            small_exon_threshold,
            small_exon_threshold1,
        ...
        if len(novelisoform_dict) > 0:
            ...
            novelisoform_dict = dict(
                sorted(novelisoform_dict.items(), key=lambda item: -len(item[1]))
            )
        else:
            for ii in range(len(Read_novelIsoform)):
                Read_Isoform_compatibleVector.append(
                    (Read_novelIsoform[ii][0], [0] * n_isoforms)
                )
            read_novelisoform_tuples = []
            novelisoform_dict = {}
...
def find_novel_by_chunk(
    df_assign, exonInfo, small_exon_threshold, small_exon_threshold1, chunk_size=1500
):
    ...
    while i < len(df_assign_list):
        ...
        novelisoform_dict, novel_df, novel_df_empty = map_read_to_novelisoform(
            novelisoform_dict,
            assigns,
            df_assign,
            exonInfo,
    ...
    assigns = isoformInfo_to_onehot(
        novelisoform_dict, geneInfo=None, nExon=len(exonInfo)
    )
    # can allow many multiple mapping to reduce novel isoform number
    novelisoform_dict, novel_df, novel_df_empty = map_read_to_novelisoform_loss(
        novelisoform_dict,
        assigns,
        df_assign_archive,
        exonInfo,
...
def group_novel_isoform(df, geneStrand, parse=False):
    ...
    if df_novel.shape[1] > 1:
        ...
        while len(novel_isoform_id) > 0:
            ...
            novel_isoform_group_list, novel_isoform_id, parent_id = pair_isoform(
                novel_isoform_group_list, novel_isoform_id, query_id, geneStrand
            )
        novel_isoform_group_name = [
            "novelIsoform_" + str(i[-1]) for i in novel_isoform_group_list
        ]
        df_novel_group = []
        for i in range(len(novel_isoform_group_list)):
            isoform_group_name = [
                "novelIsoform_" + str(ii) for ii in novel_isoform_group_list[i]
            ]
...
def find_parent_isoform(novel_isoform_id_list, child_id, geneStrand=None):
    for i in range(len(novel_isoform_id_list)):
        ...
        if set(child_exon).issubset(set(parent_exon)):
            ...
            if (
                geneStrand == "+"
                and len([ele for ele in diff_elements if ele < min(child_exon)])
                == len(diff_elements)
            ) or (
                geneStrand == "-"
                and len([ele for ele in diff_elements if ele > max(child_exon)])
                == len(diff_elements)
                or geneStrand is None
            ):
                return novel_isoform_id_list[i]


def generate_gene_annotation_novel_isoform(
    geneID,
    isoformID,
    geneStructureInformation_path,
    gtf_path,
    output_gtf_path,
...
def get_transcript_id(attribute):
    ...
    transcript_name_part = next(
        (part for part in parts if "transcript_id" in part), None
    )
...
def add_novel_isoform_to_gtf(
    gtf_path,
    gene_id,
    gene_name,
    novel_dict_list,
    output_gtf_path,
    only_this_gene=False,
    filter_existing=None,
):
    ...
    if len(novel_dict_list) > 0:
        for novel_dict in novel_dict_list:
            ...
            for start, end in novel_exon_annotation:
                ...
                new_entry["end"] = end
                new_entry["attribute"] = (
                    f'gene_id "{gene_id}"; transcript_id "{new_transcript_id}"; gene_name "{gene_name}"; '
                )
...
def read_exon_match(read, Info_singlegene):
    ...
    overlap_length = max(
        0, max(readEnd, geneInfo["geneEnd"]) - min(readStart, geneInfo["geneStart"])
    )
...
def exon_hit(mapPositions, exonInfo):
    ...
    for i in mapPositions:
        while i >= exonInfo[exon_index][1] and exon_index < len(exonInfo) - 1:
            exon_index += 1
        if int(i) >= int(exonInfo[exon_index][0]) and int(i) < int(
            exonInfo[exon_index][1]
        ):
...
def detect_poly_parse(read, window=20, n=15):
    ...
    seq_start, seq_end = (
        max(0, read.query_alignment_start - 2 * window),
        min(read.query_alignment_start + window, read.query_alignment_end),
    )
    query_sequence0 = read.query_sequence[seq_start : (seq_end + 1)]  # head
    seq_start, seq_end = (
        max(read.query_alignment_start, read.query_alignment_end - window),
        min(read.query_alignment_end + 2 * window, len(read.seq)),
    )
...
def choose_gene_from_meta(
    ...
    truncation_match=0.5,
    pacbio=False,
):
    results = []
    for i, info_singlegene in enumerate(Info_multigenes):
        gene_name, read_coverage, gene_length, n_mapExons = read_exon_match(
            read, info_singlegene
        )
        results.append(
            {
                "geneName": gene_name,
    ...
    if df_exon.shape[0] == 0 and df_intron.shape[0] > 0:
        ...
        read_novelisoform_tuple, read_isoform_compatibleVector_tuple, mapping_scores = (
            map_read_to_gene(
                read,
                Info_multigenes[ind],
                lowest_match,
        ...
        df_exon = df_exon.sort_values(
            by=["nMapExons", "readCoverage", "geneLength"],
        ...
        if sum(df_exon.nMapExons) >= 2:
            ...
            (
                read_novelisoform_tuple_dict,
                read_isoform_compatibleVector_tuple_dict,
                readType,
                read_mapping_scores_dict,
            ) = {}, {}, [], {}
            for ind in inds:
                (
                    read_novelisoform_tuple_,
                    read_isoform_compatibleVector_tuple_,
                    mapping_scores_,
                ) = map_read_to_gene(
                    read,
                    Info_multigenes[ind],
            ...
            df_exon = df_exon.sort_values(
                by=["readType", "nMapExons", "readCoverage", "geneLength"],
                ascending=[True, False, False, False],
            )
            ind = df_exon["index"][0]
            (
                read_novelisoform_tuple,
                read_isoform_compatibleVector_tuple,
                mapping_scores,
            ) = (
                read_novelisoform_tuple_dict[ind],
                read_isoform_compatibleVector_tuple_dict[ind],
                read_mapping_scores_dict[ind],
            )
...
def choose_gene_from_meta_parse(
    read,
    Info_multigenes,
    lowest_match=0.2,
    lowest_match1=0.8,
...
def map_read_to_gene_parse(
    ...
    def generate_read_exon_map_vector(exon_map_pct):
        ...
        exon_map_vector_mapped = [
            1 if pct >= upper_match else 0 for pct in exon_map_pct
        ]
        exon_map_vector_skipped = [
            -1 if pct <= lower_match else 0 for pct in exon_map_pct
        ]
        exon_map_vector_notrunct = [
            mapped + skipped
            for mapped, skipped in list(
                zip(exon_map_vector_mapped, exon_map_vector_skipped)
            )
        ]
        if poly:
            if geneInfo["geneStrand"] == "+":
                index_of_one = (
                    exon_map_vector_notrunct.index(1)
                    if 1 in exon_map_vector_notrunct
                    else len(exon_map_vector_notrunct)
                )
                exon_map_vector_trunct = [
                    0 if i < index_of_one else exon_map_vector_notrunct[i]
                    for i in range(len(exon_map_vector_notrunct))
                ]
            else:  # "-"
                last_index_of_one = (
                    len(exon_map_vector_notrunct)
                    - 1
                    - exon_map_vector_notrunct[::-1].index(1)
                    if 1 in exon_map_vector_notrunct
                    else -1
                )
                exon_map_vector_trunct = [
                    exon_map_vector_notrunct[i] if i <= last_index_of_one else 0
                    for i in range(len(exon_map_vector_notrunct))
                ]
        else:
            if 1 in exon_map_vector_notrunct:
                ...
                exon_map_vector_trunct = [
                    exon_map_vector_notrunct[i]
                    if index_of_one <= i <= last_index_of_one
                    else 0
                    for i in range(len(exon_map_vector_notrunct))
                ]
            else:
    ...
    qualifyExon = [
        i for i, (a, b) in enumerate(exonInfo) if b - a >= small_exon_threshold
    ]
    ...
    if len(mapExons) > 0:
        ...
        exon_map_pct = [
            round(base / (exonInfo[i][1] - exonInfo[i][0]), 2) if base else 0
            for i, base in enumerate(exon_map_base)
        ]
        ...
        if poly:
            if (
                geneInfo["geneStrand"] == "+"
            ):  # check if the first non-zero elements are > truncation_match, if yes, change to 1, otherwise keep unchanged
                for i, pct in enumerate(exon_map_pct):
                    if pct > 0 and (
                        pct >= truncation_match or exonLength[i] * pct >= 100
                    ):
                        exon_map_pct[i] = 1
                        break
                    elif pct > 0:
            ...
            for i in range(len(exon_map_pct_right) - 1, -1, -1):
                if exon_map_pct_right[i] > 0 and (
                    exon_map_pct_right[i] >= truncation_match
                    or exonLength[i] * exon_map_pct_right[i] > 100
                ):
        ...
        exon_map_vector_trunct_conservative = [
            vec if i in qualifyExon else 0
            for i, vec in enumerate(exon_map_vector_trunct)
        ]
        read_exon_map_list = exon_map_vector_trunct_conservative
        read_isoform_compatibleVector = map_read_to_isoform_known(
            isoform_exon_map_list,
            read_exon_map_list,
            exon_map_pct,
            lowest_match1,
            lowest_match,
        ...
        if sum(read_isoform_compatibleVector) == 0:
            threshold = max(
                small_exon_threshold, min(small_exon_threshold1, np.mean(exonLength))
            )
            for new_threshold in range(
                small_exon_threshold + 10, int(threshold) + 1, 10
            ):
                # read_exon_map_list = exon_map_vector_trunct
                qualifyExon_new = [
                    i for i, (a, b) in enumerate(exonInfo) if b - a >= new_threshold
                ]
                read_exon_map_list = [
                    vec if i in qualifyExon_new else 0
                    for i, vec in enumerate(exon_map_vector_trunct)
                ]
                read_isoform_compatibleVector_secondary = map_read_to_isoform_known(
                    isoform_exon_map_list,
                    read_exon_map_list,
                    exon_map_pct,
                    lowest_match1,
                ...
                if sum(read_isoform_compatibleVector_secondary) > 0:
                    read_isoform_compatibleVector = (
                        read_isoform_compatibleVector_secondary
                    )
    ...
    for i, isoform_name in enumerate(isoform_names):
        if read_isoform_compatibleVector[i] == 0:
            ...
            exon_lengths = [
                (b - a) for k, (a, b) in enumerate(exonInfo) if k in exon_index_list
            ]
...
def map_read_to_gene(
    read,
    Info_singlegene,
    lowest_match=0.2,
    lowest_match1=0.6,
    small_exon_threshold=0,
    ...
    if len(mapExons) > 0:
        ...
        # do not consider non-qualified exons
        isoform_exon_map_list = isoformInfo_to_onehot(isoformInfo, geneInfo)
        exon_map_vector_trunct_conserve = [
            vec if i in qualifyExon else 0
            for i, vec in enumerate(exon_map_vector_trunct)
        ]
...
def map_read_to_isoform_known(
    isoform_exon_onehot_list,
    read_exon_map_list,
    exon_map_pct,
    threshold_high,
    threshold_low,
    ...
    def map_read_to_isoform_known_(isoform_exon_onehot_list, read_exon_map_list):
        ...
        conflict_matrix = np.tensordot(
            isoform_ones, reads_minus_ones, axes=([1], [1])
    ...
    compatible_matrix, isoform_ones = map_read_to_isoform_known_(
        isoform_exon_onehot_list, read_exon_map_list
    )
    # multiple mapping
    ...
    if compatible_matrix.sum() > 1:
        ...
        transformed_read_pct = np.where(
            read_pct > threshold_high,
            1,
            np.where(
                read_pct < threshold_low,
                0,
...
def compile_compatible_vectors(
    ...
    if len(Read_novelIsoform_polished) > 0:
        read_novelisoform_df = pd.DataFrame(
            Read_novelIsoform_polished, columns=["read", "isoform"]
        )
        read_novelisoform_df_expanded = pd.crosstab(
            read_novelisoform_df["read"], read_novelisoform_df["isoform"]
        )
        isoform_pct = (
            read_novelisoform_df_expanded.sum(axis=0)
            / read_novelisoform_df_expanded.shape[0]
        )
        ...
        novel_read_novelisoform_df_expanded = np.zeros_like(
            read_novelisoform_df_expanded.values
        )
        novel_read_novelisoform_df_expanded[
            np.arange(len(read_novelisoform_df_expanded)), best_isoform_indices
        ] = 1
        novel_read_novelisoform_df_expanded = pd.DataFrame(
            novel_read_novelisoform_df_expanded,
            index=read_novelisoform_df_expanded.index,
            columns=read_novelisoform_df_expanded.columns,
        )
        columns_to_keep = novel_read_novelisoform_df_expanded.columns[
            novel_read_novelisoform_df_expanded.sum(axis=0) > 0
        ]
        novel_read_novelisoform_df_expanded = novel_read_novelisoform_df_expanded[
            columns_to_keep
        ]
    # combine existing/uncharacterized isoforms with novel
    read_annoisoform_df = pd.DataFrame.from_dict(
        dict(Read_knownIsoform_polished),
        orient="index",
        columns=geneInfo["isoformNames"],
    )
    if novel_read_novelisoform_df_expanded is not None:
        Read_Isoform_df = (
            read_annoisoform_df.merge(
                novel_read_novelisoform_df_expanded,
                how="outer",
                left_index=True,
                right_index=True,
    ...
    if (Read_Isoform_df.sum(axis=1) == 0).any():
        Read_Isoform_df["uncategorized"] = (Read_Isoform_df.sum(axis=1) == 0).astype(
            int
        )
...
def save_compatibleVector_by_gene(
    geneName,
    geneID,
    geneChr,
    colNames,
    ...
    logger.info("gene " + str(geneName) + " processing")
    readmapping_data = []
    logger.info("ENTERING ISOFORM")
    logger.info(colNames)
    logger.info(Read_Isoform_compatibleVector)
    logger.info(Read_knownIsoform_scores)
    if Read_Isoform_compatibleVector is not None:
            ...
            for idx, value in enumerate(indicators):
                if value == 1:  # Only consider mappings where the indicator is 1
                    if qname_cbumi_dict is not None:
                        if parse:
                            cb, umi = (
                                "_".join(qname_cbumi_dict[readname].split("_")[:3]),
                                qname_cbumi_dict[readname].split("_")[3],
                            )
                        else:
                            cb, umi = (
                    ...
                    if isoform_name != "uncategorized":
                        ...
                        exon_coords = merge_exons(
                            [
                                (exonInfo[ind][0], exonInfo[ind][1])
                                for ind in exon_indices
                            ]
                        )
                        exon_coords = ",".join(f"{exon}" for exon in exon_coords)
                        readmapping_data.append(
                            [
                                readname,
                                isoform_name,
                                ",".join(map(str, exon_indices)),
                                exon_coords,
                                cb,
                                umi,
                                cbumi,
        ...
        output = dict(
            {
                "Gene": geneName,
                "compatibleMatrix": mat,
                "rowNames": rowNames,
                "colNames_isoforms": colNames,
            }
        )
    else:
        rowNames = []
    ...
    logger.info(rowNames)
    logger.info(output_folder)
    if output_folder is not None and len(rowNames) > 0:
        data_df = pd.DataFrame(
            output["compatibleMatrix"],
            index=output["rowNames"],
            columns=output["colNames_isoforms"],
        )
        ...
        if len(readmapping_data) > 0:
            ...
            scores_df = pd.DataFrame.from_dict(
                Read_knownIsoform_scores,
                orient="index",
                columns=list(isoformInfo.keys()),
            )
            scores_long_df = scores_df.reset_index().melt(
                id_vars="index", var_name="Isoform", value_name="MappingScore"
            )
            scores_long_df.rename(columns={"index": "Read"}, inplace=True)
            readmapping = pd.merge(
                readmapping, scores_long_df, on=["Read", "Isoform"], how="left"
            )
            readmapping_filename = os.path.join(
                output_folder0, geneName + "_read_isoform_exon_mapping.tsv"
            )
        ...
        output_folder is not None and len(rowNames) == 0
    ):  # log the gene because no reads mapped
        output_folder = os.path.join(output_folder, "compatible_matrix")
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
...
def map_read_to_novelisoform_loss(
    novelisoform_dict,
    assigns,
    df_assign_archive,
    exonInfo,
    ...
    def map_read_to_isoform_(df_assign_archive):
        ...
        minus_ones = (data == -1).astype(int)
        conflict_matrix = np.tensordot(
            novel_ones, minus_ones, axes=([1], [1])
        ) + np.tensordot(novel_minus_ones, ones, axes=([1], [1]))
        compatible_matrix = (conflict_matrix == 0).astype(int).transpose()
...
def map_read_to_novelisoform(
    isoformInfo_dict,
    isoform_assignment_vector_list,
    read_assignment_df,
    exonInfo,
    small_exon_threshold=0,
    small_exon_threshold1=80,
    ...
    for small_exon_threshold in small_exon_thresholds:  # can cause multiple mapping
        qualify_exon_indicator = [
            1 if b - a > small_exon_threshold else 0 for a, b in exonInfo
        ]
        read_assignment_df_empty_filtered = (
            read_assignment_df_empty * qualify_exon_indicator
        )
        novel_df_, novel_df_empty = map_read_to_isoform_(
            read_assignment_df_empty_filtered
        )
        read_assignment_df_empty = read_assignment_df.loc[novel_df_empty.index]
        novel_df = pd.concat([novel_df, novel_df_])
    read_novelisoform_tuples = [
        (row, col) for (row, col), value in novel_df.stack().items() if value == 1
    ]
    novel_isoform_names = list(set([b for a, b in read_novelisoform_tuples]))
    novelisoform_dict = {
        key: value
        for key, value in isoformInfo_dict.items()
        if key in novel_isoform_names
    }
    novelisoform_dict = dict(
        sorted(novelisoform_dict.items(), key=lambda item: -len(item[1]))
    )
