...
parser.add_argument(
    "--task",
    type=str,
    help="choose task from annotation, compatible matrix, count matrix, summary, or all; or visualization",
)
parser.add_argument(
    "--platform",
    type=str,
    default="10x-ont",
    help="platform: 10x-ont, parse-ont, or 10x-pacbio",
)
parser.add_argument(
    "--target", type=str, nargs="+", help="path to target root folders for output files"
)  # a list
parser.add_argument(
    "--bam",
    type=str,
    nargs="+",
    help="one or multiple bam file paths or bam folder paths",
)  # a list
parser.add_argument(
    "--build",
    type=str,
    help="genome build, bam files for parse platform have contigs in the format of build_chr1",
)
# task is annotation
parser.add_argument(
    "--reference",
    type=str,
    default="/scr1/users/xu3/singlecell/ref/refdata-gex-GRCh38-2020-A/genes/genes.gtf",
    help="Path to gene annotation file in gtf format. Set it as None if using annotation-free mode. To save time, users can use SCOTCH generated annotation file usint the argument of --reference_pkl. But the gtf file is still needed.",
)
parser.add_argument(
    "--reference_pkl",
    type=str,
    default="../data/geneStructureInformation.pkl",
    help="Path to gene annotation file generated by SCOTCH named with geneStructureInformation.pkl file. Set this argument and --reference as None if using annotation-free mode. Set this argument to None and set --reference to use known annotation file, then SCOTCH will generate the annotation pickle file. Default is in human hg38 build",
)
parser.add_argument(
    "--update_gtf",
    action="store_true",
    help="use bam file to update existing gtf annotations",
)
parser.add_argument(
    "--update_gtf_off",
    action="store_false",
    dest="update_gtf",
    help="do NOT use bam file to update existing gtf annotations",
)
parser.add_argument(
    "--coverage_threshold_gene",
    type=int,
    default=20,
    help="coverage threshold to support gene discovery",
)
parser.add_argument(
    "--coverage_threshold_exon",
    type=float,
    default=0.02,
    help="coverage threshold to support exon discovery, percentage to the maximum coverage",
)
parser.add_argument(
    "--coverage_threshold_splicing",
    type=float,
    default=0.02,
    help="threshold to support splicing discovery, percentage to the maximum splicing junctions",
)
parser.add_argument(
    "--z_score_threshold",
    type=int,
    default=10,
    help="threshold to support exon coverage sharp change discovery",
)
parser.add_argument(
    "--min_gene_size",
    type=int,
    default=50,
    help="minimal length of novel discovered gene",
)
...
parser.add_argument(
    "--no_bam_annotation",
    action="store_true",
    help="Only create annotation reference files. No BAM annotation.",
)
...
parser.add_argument(
    "--cover_existing_false", action="store_false", dest="cover_existing"
)
parser.add_argument(
    "--small_exon_threshold",
    type=int,
    default=0,
    help="dynamic exon length threshold to ignore for includsion and exclusion",
)
parser.add_argument(
    "--small_exon_threshold_high",
    type=int,
    default=80,
    help="the upper bound of dynamic exon length threshold to ignore for includsion and exclusion",
)
parser.add_argument(
    "--truncation_match",
    type=float,
    default=0.4,
    help="higher than this threshold at the truncation end will be adjusted to 1",
)
parser.add_argument(
    "--match_low",
    type=float,
    default=0.1,
    help="the base percentage to call a read-exon unmatched",
)
parser.add_argument(
    "--match_high",
    type=float,
    default=0.6,
    help="the base percentage to call a read-exon matched",
)
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="filter out novel isoforms with supporting read number smaller than n",
)
parser.add_argument(
    "--group_novel",
    action="store_true",
    help="whether to further group novel isoforms generated in compatible matrix, default is true",
)
parser.add_argument("--group_novel_off", action="store_false", dest="group_novel")
parser.add_argument(
    "--save_csv",
    action="store_true",
    help="whether to save count matrix output as csv format",
)
parser.add_argument("--save_csv_false", action="store_false", dest="save_csv")
parser.add_argument(
    "--save_mtx",
    action="store_true",
    help="whether to save count matrix output as mtx format",
)
...
parser.add_argument(
    "--single_cell",
    action="store_true",
    help="default setting for preprocessing single cell data",
)
parser.add_argument(
    "--bulk", action="store_false", dest="single_cell", help="bulk data"
)
...
parser.add_argument(
    "--sample_names", type=str, nargs="+", help="sample names for visualization"
)  # a list
parser.add_argument(
    "--novel_pct",
    type=float,
    default=0.1,
    help="only keep novel isoform annotation if expression in count matrix surpass the threshold",
)
parser.add_argument(
    "--junction_num", type=int, default=10, help="only keep junctions over this number"
)
parser.add_argument("--width", type=int, default=12)
parser.add_argument("--height", type=float, default=1)
parser.add_argument(
    "--annotation_scale", type=float, default=0.25, help="annotation plot scale"
)
...
def main():
    ...
    def run_annotation():
        ...
        logger.info(
            f"Coverage threshold (splicing): {args.coverage_threshold_splicing}"
        )
        ...
        annotator = annot.Annotator(
            ...
            reference_pkl_path=args.reference_pkl,
            bam_path=args.bam,
            update_gtf=args.update_gtf,
            workers=args.workers,
            coverage_threshold_gene=args.coverage_threshold_gene,
    ...
    def run_compatible():
        ...
        logger.info(
            f"Start generating compatible matrix step for all targets. Job: {args.job_index}"
        )
        ...
        logger.info(
            f"Small exon threshold (low): {args.small_exon_threshold}. Job: {args.job_index}"
        )
        logger.info(
            f"Small exon threshold (high): min(average exon length, {args.small_exon_threshold_high}). Job: {args.job_index}"
        )
        logger.info(
            f"Truncation match: {args.truncation_match} or 100bps. Job: {args.job_index}"
        )
        ...
        readmapper = cp.ReadMapper(
            target=args.target,
            bam_path=args.bam,
            lowest_match=args.match_low,
            lowest_match1=args.match_high,
        ...
        logger.info(
            f"Completed generating compatible matrix for all targets.  Job: {args.job_index}"
        )
        copy_log_to_targets(log_file, args.target)
    ...
    def run_count():
        ...
        countmatrix = cm.CountMatrix(
            target=args.target,
            novel_read_n=args.novel_read_n,
            platform=args.platform,
            workers=args.workers,
        ...
        if args.platform == "parse":
            assert len(args.target) == 1, (
                "Error: The length of target must be 1 when platform is 'parse'."
            )
    ...
    def run_summary():
        ...
        logger.info(
            "Start summarizing read isoform mapping and annotations for all targets."
        )
        logger.info(f"Target directories: {args.target}")
        for i in range(len(args.target)):
            logger.info(f"Start summarizing annotation for target: {args.target[i]}")
            cp.summarise_annotation(args.target[i], logger)
            logger.info(
                f"Start summarizing read mapping information for target: {args.target[i]}"
            )
            cp.summarise_auxillary(args.target[i], logger)
        logger.info(
            "Completed summarizing annotations and auxiliary information for all targets."
        )
        copy_log_to_targets(log_file, args.target)
